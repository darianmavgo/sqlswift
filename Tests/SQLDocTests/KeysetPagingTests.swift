...
final class KeysetPagingTests: XCTestCase {
    func testKeysetPaging() throws {
        ...
        XCTAssertEqual(page1.rows.count, 100)
        XCTAssertEqual(page1.rowIDs.first, 1)
        XCTAssertEqual(page1.rowIDs.last, 100)
        XCTAssertEqual(page1.path, "interpolated")
        ...
        XCTAssertEqual(page2.rows.count, 100)
        XCTAssertEqual(page2.rowIDs.first, 101)
        XCTAssertEqual(page2.rowIDs.last, 200)
        XCTAssertEqual(page2.path, "keyset")

        // Jump to offset 500
        let page3 = try doc.rows(window: Window(table: "sensor_readings", limit: 50, offset: 500))
        XCTAssertEqual(page3.rows.count, 50)
        XCTAssertEqual(page3.rowIDs.first, 501)
        XCTAssertEqual(page3.path, "interpolated")

        // Sort by temp DESC
        let page4 = try doc.rows(window: Window(table: "sensor_readings", limit: 20, sort: "temp", desc: true))
        XCTAssertEqual(page4.rows.count, 20)
        XCTAssertEqual(page4.path, "sorted-offset")
    }
}
